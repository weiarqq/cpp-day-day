...
#### 回调函数
在 C++ 中，回调函数是指通过函数指针（或其他可调用对象）将函数作为参数传递给另一个函数，以便在特定事件发生或条件满足时被调用。它广泛用于事件处理、异步操作、框架设计等场景。


**1. 基本原理：函数指针**
回调函数的核心是**函数指针**——指向函数的指针变量，通过它可以间接调用函数。
...
// 接收回调函数的函数（通过函数指针参数）
void process(int x, int y, void (*callback)(int, int)) {
    // 执行某些操作后，调用回调函数
    callback(x, y); // 触发回调
}
...
int main() {
    // 传递回调函数给 process
    process(3, 5, addAndPrint); // 输出：Sum: 8
    return 0;
}
```
...
**2. 类成员函数作为回调函数**
普通函数指针无法直接指向类的非静态成员函数（因为成员函数包含隐含的 `this` 指针）。需通过以下方式解决：

 方法 1：使用静态成员函数
静态成员函数没有 `this` 指针，可直接作为回调：
```cpp
#include <iostream>
...
int main() {
    process(4, 5, Calculator::multiply); // 输出：Product: 20
    return 0;
}
```

方法 2：结合 `std::function` 和 `std::bind`（C++11 及以上）
对于非静态成员函数，可通过 `std::bind` 绑定 `this` 指针，再用 `std::function` 包装：
```cpp
#include <iostream>
...
public:
    // 非静态成员函数
    void printResult(int a, int b) const {
        std::cout << "Result: " << a + b << std::endl;
    }
};

// 接收 std::function 作为回调（更灵活）
void process(int x, int y, const std::function<void(int, int)>& callback) {
    callback(x, y);
}

int main() {
    Printer printer;
    // 绑定非静态成员函数和对象（this 指针）
    auto callback = std::bind(&Printer::printResult, &printer, std::placeholders::_1, std::placeholders::_2);
    process(2, 3, callback); // 输出：Result: 5
    return 0;
}
```


**3. 现代 C++ 中的回调：lambda 表达式**
C++11 引入的 lambda 表达式可直接作为匿名回调函数，简化代码：
```cpp
#include <iostream>
#include <functional>
...
int main() {
    // 直接传递 lambda 作为回调
    process(10, 20, [](int a, int b) {
        std::cout << "Difference: " << a - b << std::endl; // 输出：Difference: -10
    });
    return 0;
}
```
...
**4. 回调函数的典型应用场景**
- **事件驱动编程**：如 GUI 按钮点击事件回调。
- **异步操作**：如网络请求完成后的回调处理。
- **算法框架**：如排序函数中自定义比较器（如 `std::sort` 的第三个参数）。
- **钩子（Hook）机制**：在框架中预留回调接口，允许用户自定义扩展。


**总结**
- 基础回调通过**函数指针**实现，适用于普通函数或静态成员函数。
- 非静态成员函数需结合 `std::function` 和 `std::bind` 绑定 `this` 指针。
- 现代 C++ 中，**lambda 表达式**是最简洁的回调方式。
- 回调函数的核心思想是“将函数作为参数传递，在特定时机执行”。
...
`& `:按位与,只有对应位都为 1 时，结果位才为 1；否则为 0

`|`:按位或,只要对应位有一个为 1，结果位就为 1；只有都为 0 时才为 0

`^`:按位异或,对应位不同时结果位为 1，相同时结果位为 0
...
在 C++ 中，当用户没有显式定义某些成员函数时，编译器会根据类的情况自动生成默认版本的函数。这些默认函数包括以下 6 种：

1. **默认构造函数（Default Constructor）**  
   - 无参数的构造函数，用于创建对象时初始化成员（如内置类型默认初始化，类类型调用其默认构造函数）。  
   - 仅当类中**没有显式定义任何构造函数**时，编译器才会生成默认构造函数。


2. **默认析构函数（Default Destructor）**  
   - 无参数，用于对象销毁时清理资源（如释放动态内存、调用成员对象的析构函数等）。  
   - 编译器总是会生成默认析构函数（除非显式定义），默认行为是**非虚函数**（若类作为基类，通常需要显式定义为虚析构函数）。


3. **拷贝构造函数（Copy Constructor）**  
   - 以同类对象的**常量引用**为参数（`Class(const Class&)`），用于通过已有对象创建新对象时的“深拷贝”（默认行为是**浅拷贝**，即逐个复制成员变量）。  
   - 当用户**没有显式定义拷贝构造函数**时，编译器会生成。若类包含指针等资源，浅拷贝可能导致问题（需手动定义）。


4. **拷贝赋值运算符（Copy Assignment Operator）**  
   - 函数原型为 `Class& operator=(const Class&)`，用于将一个对象赋值给另一个已存在的对象（默认行为是**浅拷贝**）。  
   - 当用户**没有显式定义**时，编译器会生成。同样，含资源的类需手动定义以避免浅拷贝问题。


5. **移动构造函数（Move Constructor，C++11 新增）**  
   - 以同类对象的**右值引用**为参数（`Class(Class&&)`），用于通过“即将销毁”的临时对象创建新对象，实现资源的“移动”而非拷贝，提高效率。  
   - 仅当类中**没有显式定义拷贝构造、拷贝赋值、移动赋值，且没有显式定义析构函数**时，编译器才会自动生成。


6. **移动赋值运算符（Move Assignment Operator，C++11 新增）**  
   - 函数原型为 `Class& operator=(Class&&)`，用于将临时对象的资源移动给已存在的对象。  
   - 生成条件与移动构造函数相同：需满足没有显式定义拷贝相关函数、移动赋值和析构函数。
   **注意：**

- C++11 引入了 `=default` 和 `=delete` 语法，可显式指定生成默认函数或禁用默认函数（如 `Class() = default;` 强制生成默认构造，`Class(const Class&) = delete;` 禁用拷贝构造）。
- 当类中包含指针、动态内存等资源时，默认的拷贝/移动函数可能无法正确管理资源，需手动实现以避免内存泄漏或悬挂指针。
